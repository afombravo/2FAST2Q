...
def input_getter_txt(path):

    """ Handles the parsing of the .txt only parameters"""
    
    inputs = os.path.join(path)
...
def initializer(arg):
    ...
    if arg == "g":
        ...
        path=sys.argv[2]
        folder_path, guides, out, extension, mismatch, phred, start, lenght, ram = input_getter_txt(path)
...
def main(arg):
    ...
    version,phred,separator,start, lenght, ram, extension = initializer(arg)
